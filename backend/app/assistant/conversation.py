...
A guest assistant is not a search box: "hi", "how can you help?", "thanks" and "bye" are part of a
normal conversation and must never be answered with "I couldn't find that in our hotel information".

These four intents are handled here, before any retrieval, model call or tool:
...
ConversationalIntent = Literal["greeting", "capability", "thanks", "goodbye"]
...
def reply_for(intent: ConversationalIntent, kb: KnowledgeBase, suggestions: list[str]) -> ChatReply:
    """A natural reply that states no hotel facts beyond the property's own name."""
    hotel = kb.hotel.name
    if intent == "greeting":
        text = (
            f"Hello! Welcome to {hotel}. 👋\n\n"
            "I can help with rooms, amenities, breakfast, hotel policies and room availability.\n\n"
            "What would you like to know?"
        )
    elif intent == "capability":
        text = (
            f"I'm the guest assistant for {hotel}. I can help with:\n\n"
            "• Rooms and room features\n"
            "• Amenities and facilities\n"
            "• Breakfast and dining\n"
            "• Check-in and check-out times\n"
            "• Hotel policies, including cancellation\n"
            "• Room availability for your dates\n\n"
            'Just ask in your own words, for example "Is breakfast included?" or '
            '"Do you have rooms for 3 adults next weekend?"'
        )
    elif intent == "thanks":
        text = "You're welcome! Let me know if there's anything else I can help with about your stay."
    else:
        text = f"Thank you for visiting {hotel}. We hope to welcome you soon — have a lovely day! 🌴"
    return ChatReply(type="clarification", text=text, suggestions=suggestions if intent in ("greeting", "capability") else [])
