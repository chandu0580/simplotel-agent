...
| **Hotel profile request (`GET /api/v1/hotels/{id}`) fails** | The header uses built-in defaults (hotel and assistant name, default colour, English only) and chat still works. The landing page falls back to the default name and hides the room and contact sections rather than showing empty ones. |
