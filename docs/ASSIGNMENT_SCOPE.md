...
| Shared state and data | In-memory stores by default; optional `STATE_BACKEND=redis` adapter for conversations, rate limits, idempotency and locks across replicas; optional PostgreSQL adapter with schema, row-level security, migrations, audit sink and retention job (other repositories schema only). Their integration tests were verified locally once and are not run in CI |
...
| Delivery | Local run with a Python virtual environment (`uvicorn`) and the Vite dev server; GitHub Actions CI (backend, security, frontend, e2e) plus a manual live-AI eval workflow (neither has been run on GitHub yet). Docker/containerization: NOT REQUIRED FOR CURRENT PROJECT — removed intentionally (a container setup was added at one point and later removed as out of scope). |
