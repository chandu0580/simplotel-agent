...
- **Implemented for the hosted demo (hosting requirement elsewhere).** `Content-Security-Policy` (`default-src 'self'; script-src 'self'; frame-ancestors 'none'`, with `style-src 'unsafe-inline'` for brand colours), `Permissions-Policy` (camera, microphone, geolocation, payment, USB denied) and HSTS at TLS termination must be set by whatever serves the built SPA in a real deployment. The values are in [DEPLOYMENT.md](DEPLOYMENT.md) and are set by [`frontend/vercel.json`](../frontend/vercel.json), verified on the deployed site. No automated test asserts them, and a host other than the demo's Vercel project must set them itself.
