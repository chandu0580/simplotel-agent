...
- **Hosting requirement (implemented for the hosted demo):** whatever serves the built SPA must set `Content-Security-Policy`, `Permissions-Policy` and, at TLS termination, `Strict-Transport-Security`; `frontend/vercel.json` does this for the demo. `/metrics` must not be public: the demo has no internal network, so it sets `METRICS_ENABLED=false` instead of exposing it. If it sits in front of the API as a reverse proxy, it must overwrite `X-Forwarded-For` before the backend is run with `TRUST_PROXY_HEADERS=true`.
...
3. **`answer_guest`**: `ModelReply` validation (`type` must be answer, clarification or fallback, `text` non-empty, `source_ids` and `suggestions` lists). Failure raises `LLMError("invalid_output")`. On success, `OutputGuardrails.check_answer` runs these checks in order:
   ...
   - a currency amount found neither in the cited entries nor among the prices an availability search already showed this guest (`Conversation.recent_offers`) becomes a fallback (`unsupported_price`), when `guardrail_price_check_enabled` is on. Search prices are seasonal and differ from the indicative knowledge-base rates, so without them a correct answer repeating what the guest just saw would be blocked. `AIAssistant._answer` evaluates this flag per call with the tenant's overrides, and the container-level value is only the default;
