...
- **Security headers:** the backend middleware sets `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Cache-Control: no-store`, and HSTS in production. `Content-Security-Policy`, `Permissions-Policy` and HSTS for the SPA are a hosting requirement, implemented for the hosted demo in `frontend/vercel.json` and verified on the deployed site. Any other host serving the built SPA must set them itself.
