...
│      │               │  output: JSON {type, text, source_ids,     │
│      │               │                suggestions}                │
│      │               ▼                                            │
...
| `claude_assistant.py` | One model call per turn with tools and structured output, followed by deterministic checks on the result (see below). |
...
3. Claude is called with:
   ...
   - both tools, with strict schemas;
   - a JSON-schema output format;
   - `effort: low`;
   - server-side refusal fallbacks (`fallbacks: "default"`).
4. What comes back determines the reply:
   - **Any tool call:** arguments are validated with Pydantic first. Wrong types or an unknown tool raise `LLMError`, which means offline fallback, not a 500. Remembered booking dates fill in only when the model gave **no** dates, so a new check-in is never paired with an old check-out.
   ...
   - **JSON answer:** each `source_id` is checked against the knowledge base, and unknown ids are dropped and logged. An `answer` left with no valid source becomes a fallback. Every fallback gets the front-desk contact details appended.
